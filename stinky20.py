...
async def heath(ctx):
    """
    random heathcliff comic
    """
    print(f"heath called by '{ctx.message.author.name}' on '{time.asctime()}'")
    garfield_request = requests.get('https://www.gocomics.com/random/heathcliff')
    if garfield_request.status_code == 200:
        soup = bs4.BeautifulSoup(garfield_request.text, 'html.parser')
        date = str(soup.title)[7:-8]
        for meta_line in soup.find_all('meta'):
